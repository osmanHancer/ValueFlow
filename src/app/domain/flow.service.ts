...
export class FlowService {
  ...
  public nodeSaveData(nodeId:any,data:string){
    this.flow.nodes.map((node:any) => (node.id == nodeId  ? node.data=data : node.data=node.data));
    }
    ...
    public saveConnectionİcon(outputId: string, oldInputId: string,icon:string){
      var connection = this.flow.connections.find(connection =>connection.from === outputId && connection.to === oldInputId);
